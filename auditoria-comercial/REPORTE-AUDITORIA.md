...
| **Veredicto** | **NO-GO** para producción comercial · Puntuación **42 / 100** · Confianza **alta** en el bloque técnico, **media** en el normativo |
...
El veredicto es NO-GO por dos defectos P0, cada uno suficiente por sí solo:
...
Ninguno de estos defectos es de arquitectura. Los dos P0 se corrigen en días, no en trimestres, y el andamiaje para hacerlo bien ya está escrito.

---

## 2. Alcance y limitaciones

**Se ejecutó realmente:** compilación completa, las cinco suites de prueba, el servidor de desarrollo, veinte sondas de seguridad contra la API de IA en vivo, y trazado del código de cálculo, dimensionado de vidrio, lista de corte y costeo.

**No se ejecutó, y por qué:**

- **Recorrido del Workspace interno en navegador (T-055).** El candado interno exige contraseña. El auditor no introduce contraseñas en ningún campo, por regla propia, aun disponiendo de ella. El Workspace se auditó por código y por sus pruebas automatizadas, que sí cubren el candado y el cierre del catálogo.
...
Matriz completa en `MATRIZ-PRUEBAS.csv` (60 casos). Evidencia de IA en `evidencias/IA-seguridad.txt`. Registros en `logs/`.

**El punto importante sobre la cobertura:** las 215 pruebas son de calidad alta, pero **ninguna cubre la medida de vidrio, la longitud de corte, los ángulos ni la lista de materiales**. Las nueve pruebas de costeo cubren merma, mano de obra, barras, gastos fijos y escalado. Los dos defectos de fabricación más caros viven exactamente en la zona sin cobertura, y por eso llevan ahí sin que nadie los viera.
...
|---|---|---|
| **P0 Bloqueante** | 2 | D-01, D-02 |
| **P1 Crítico** | 6 | D-03 a D-08 |
...
1. Dimensionado de vidrio derivado del perfil y del rol de acristalamiento, con holgura explícita y **una sola** función compartida (D-01, D-10).
...
|---|---|---|---|
| Funcionalidad completa | 20 | **11** | Diseño y cotización sólidos; producción, inventario y usuarios ausentes |
...
1. Derivar la medida de vidrio del perfil y del rol de acristalamiento, con una sola fuente de verdad **(P0)**.
...
|---|---|---|
| T-055 Workspace en navegador | El auditor no introduce contraseñas | Contraseña desechable en entorno local, o el gate de desarrollo en loopback ya presente en el árbol |
