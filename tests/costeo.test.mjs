...
// lib/calc.ts is TypeScript and pulls in the `@/` path alias, so bundle it in-memory rather
// than depending on `npm run build` having run first — these are pure arithmetic assertions
// on the costing engine and shouldn't need the whole Sites artifact.
let calc;
before(async () => {
  const root = new URL("../", import.meta.url).pathname;
  const result = await build({
    entryPoints: [`${root}lib/calc.ts`],
    bundle: true,
    write: false,
    platform: "node",
    format: "esm",
  ...
  calc = await import(`data:text/javascript;base64,${Buffer.from(code).toString("base64")}`);
});

const spec = (mallorquina = false) => ({
  state: "Móvil", opening: "", direction: "", hardware: "", handle: "", glass: "", notes: "",
  mallorquina, sides: {}, glassSides: {},
...
// CORREDERA 60MM in white with DVH 24 mm — the default configuration of the public quoter.
const sys = { name: "CORREDERA 60MM", frame: 74, sash: 75, hardware: 950 };
...
test("el conteo de barras usa la misma longitud que el optimizador de corte", () => {
  const q = quote();
  const lineal = q.frameM + q.sashM;
  const barM = calc.BAR_LENGTH_MM / 1000;

  assert.equal(calc.BAR_LENGTH_MM, 5800);
  assert.equal(q.bars, Math.ceil(lineal / barM));
  assert.ok(q.bars * barM >= lineal, "las barras deben cubrir el lineal requerido");
  assert.ok(q.waste >= 0 && q.waste < barM, "el remanente debe caber en una barra");
});
